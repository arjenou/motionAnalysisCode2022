        ...
        if _:
            ...
            for t in online_targets:
                ...
                if tlwh[2] * tlwh[3] > min_box_area and not vertical:
                    ...
                    idfile = f"./{DEFAULT_SAVEPATH}/{tid}"
                    if os.path.exists(idfile):
                      timeVideo =str(int(cap.get(cv2.CAP_PROP_POS_MSEC))).zfill(6)
                      jpg_file=f"./{DEFAULT_SAVEPATH}/{tid}/{tid}_{frame_id}_{timeVideo}.jpg"
                      cv2.imwrite(jpg_file, corp_img)
                      print(f"save{tid}in{jpg_file} successed")   
                      print(t1)                  
                    else:
                      timeVideo =str(int(cap.get(cv2.CAP_PROP_POS_MSEC))).zfill(6)
                      try:
                        os.makedirs(f"./{DEFAULT_SAVEPATH}/{tid}")
